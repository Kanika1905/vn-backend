...
export const createProduct = async (req, res) => {
  try {
    ...
    // 👇 Grab uploaded image URLs from Cloudinary (req.files is set by multer)
    const images = (req.files || []).map((file) => file.path);
    // file.path = the full Cloudinary HTTPS URL

    const product = await Product.create({
      wholesalerId,
      categoryId,
      name,
      images,        // 👈 was `image` (single string), now `images` (array of URLs)
      description,
      price,
      quantity,
      unit,
